...
//				if R>=0, and G,B are unused, "R" will be considered the color index (like CvPlot::colors[index])
//              if none are assigned, the curve will be assigned a unique color automatically.
//
...
namespace CvPlot
{
	...
	public:
		...
		col() : R(0), G(0), B(0) {}
		col(int RRR, int GGG, int BBB) : R(RRR), G(GGG), B(BBB) {}
	};
	
	class colorlist
	{
	public:
		static std::vector<col> elements;
		
		col& operator[] (int x) {
			if(x >= elements.size()) {
				x -= elements.size() * (x / elements.size());
			}
			else if(x < 0) {
				x += elements.size() * (((-x) / elements.size()) + 1);
			}
			return elements[x];
		}
	};
