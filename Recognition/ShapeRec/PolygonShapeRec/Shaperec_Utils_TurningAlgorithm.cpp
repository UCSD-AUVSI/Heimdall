...
struct POINT {       // Cartesian point
    COORD x, y;
};
struct POLY_REC {     // polygon of n points
    int n;
    cv::Point pt[MAX_PTS];
};*/

typedef cv::Point OPENCV_POINT;
typedef int OPENCV_POINT_FLOAT;
 ...
 * discontinuity, s.
 */
void poly_to_turn_rep(std::vector<OPENCV_POINT>& p, TURN_REP* t)
{
    int n, i0, i1;
    ...
    for (theta0 = 0, i0 = 0; i0 < n; theta0 = theta1, ++i0)
    {
         ...
         */
        i1 = (i0 + 1) % n;
        dx = ((double)(p[i1].x)) - ((double)(p[i0].x));
        dy = ((double)(p[i1].y)) - ((double)(p[i0].y));
 ...
 */
static EVENT_REC eventheap[MAX_PTS+1];
...
void add_event(TURN_REP* f, TURN_REP* g, int fi, int gi)
{
    ...
    if ((t = tr_s(f, fi) - tr_s(g, gi)) > 1) return;
...
{
    int i;
    EVENT_REC next = eventheap[1];      /* remember event to return */
    EVENT_REC e = eventheap[n_events];  /* new root (before adjust) */

    --n_events;
    i = 2;
    while (i <= n_events) {
        if (i < n_events && eventheap[i].t > eventheap[i+1].t)
            ++i;
        if (e.t <= eventheap[i].t)
            break;
        else {
            eventheap[i/2] = eventheap[i];
            i *= 2;
        }
    }
    eventheap[i/2] = e;
...
ComparisonReturnedTable::ComparisonReturnedTable() : metric(0.0),
				relative_angle_to_reference(0.0),
				tr_ii(0),
				tr_ff(0),
				hht0_err(0.0),
				sslope_err(0.0) {}

ComparisonReturnedTable::ComparisonReturnedTable(double aaa, double bbb, int ccc, int ddd, double eee, double fff) : metric(aaa),
				relative_angle_to_reference(bbb),
				tr_ii(ccc),
				tr_ff(ddd),
				hht0_err(eee),
				sslope_err(fff) {}
