...
int main(int argc, char** argv)
{
	...
	if(argc >= 4) {
		...
		if(std::string(argv[3]) == "0" || std::string(argv[3]) == "1") {
			...
			std::cout << "usage:  [IMAGES DIR]  [TRUTH FILENAME]  [SAVE CROPS? BOOL 1/0]  OPTIONAL:[CROP PADDING PIXELS]  OPTIONAL:[DISPLAY IMAGES?]" << std::endl;
			return 1;
		}
		if(argc >= 5) {
		...
		cout << "usage:  [IMAGES DIR]  [TRUTH FILENAME]  [SAVE CROPS? BOOL 1/0]  OPTIONAL:[CROP PADDING PIXELS]  OPTIONAL:[DISPLAY IMAGES?]" << std::endl;
		return 0;
	}	
